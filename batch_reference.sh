	...
	do 
		...
		function run_batch {
	    	...
	    	if [[ $case != *-* ]]; then # No mixed dataset  
	        	sed -i "s|URLS_TRAIN|${basedata}_FB_Ref/${tarPrefix}-K-{1..10}_train_{0..31}.tar|g" $subBatch
	        	sed -i "s|URLS_VAL|${basedata}_FB_Ref/${tarPrefix}-K-{1..10}_val_{0..3}.tar|g" $subBatch
	        	sed -i "s|CASE|${basedata}_FB_Ref|g" $subBatch # For test combined
	        	sed -i "s|URLS_TEST|${basedata}_FB_Ref/${tarPrefix}-K-{1..10}_test_{0..3}.tar|g" $subBatch
	    	fi
