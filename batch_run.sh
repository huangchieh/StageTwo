...
#for basedata in Water-bilayer Water15_k Water-Au111 
for basedata in Water-bilayer 
# Loop 0 ----Start
do 
	...
	# Check if water on Cu(111) surface or Au(111) surface
	# And replace atomic number in template files
	if [[ $basedata == "Water15_k" ]]; then
	    subatomnumber=29 # Cu atomic number
	else
	    subatomnumber=79 # Au atomic number
	fi
	...
	if [[ $folder == "test_combined" ]]; then
        ...
        cd .. # Exit Posnet training

        echo $jobid # Echo function like return in Python
    }
    # 1. Train posnet
    ...
    #jobid3=$(run_batch test_combined batch_test_graphnet.sh None)
    # 4. Test experiments
    ...
    #jobid4=$(run_batch test_experiment batch_save_cropdata.sh None)
    # 4. Evaluate
    #jobid5=$(run_batch test_experiment batch_evaluate.sh $jobid4)
